...
`push`/`schedule`/`workflow_dispatch` are only useful for warming the cache when they run on the repository's default branch: a `pull_request_target` run's ref is always the base branch, and GitHub Actions cache entries are only restorable from the branch they were saved on (or the default branch) — a cache saved from any other branch/ref could never be restored by a `pull_request_target` run targeting the default branch anyway. The "Early checks" step therefore treats a `push`/`schedule`/`workflow_dispatch` run on a non-default ref (`refs/heads/main`/`refs/heads/master`, checked without any external call — it's derived from `github.ref`, which is already part of the job's context) the same as an empty `frozen-teams`: `skip` is set, and no Octo STS exchange or team-membership API call happens for a cache entry nothing could read back. A consuming workflow does not need to (and should not need to) guard against this itself.
