...
- New `src/resolveTeamMembership.ts` entrypoint: parses config, calls `resolveTeamMembership()` from `src/github/teams.ts`, and writes the result as a `team-membership` step output (`Map<string, Set<string>>` serialized to `JSON.stringify(Object.fromEntries(...))`, i.e. `{ "@org/team-a": ["alice", "bob"] }`).
- `src/main.ts` no longer resolves team membership itself: `EvaluateInput.orgOctokit` is replaced by a plain `teamMembership: Map<string, Set<string>>`, deserialized from the new `TEAM_MEMBERSHIP` env var (the prior step's output) rather than fetched over the network.
...
**Status: implemented, not yet reviewed.** Deviations from the plan:
- The "Early checks" step's skip condition (added in a prior follow-up for the frozen-teams-empty case) was extended to *also* skip when a configured bypass label is already on the PR, checked directly against the webhook payload's labels in Python. Without this, splitting the steps would have been a regression: today, a bypass label short-circuits `evaluate()` before it ever calls `resolveTeamMembership()`, so the per-team API calls are skipped; without extending the skip condition, the new "Resolve frozen team membership" step would run those calls unconditionally whenever `frozen-teams` is non-empty, bypass label or not. This duplicates `hasBypassLabel()`'s case-sensitive matching logic in Python, but only as a cost-saving fast path — `decide()` (`src/decision.ts`) remains the single authoritative check, so a bug in the Python duplicate could waste an API call but can never wrongly pass or wrongly fail a PR.
